...
class ToBuyerUser(models.Model):
	user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="Пользователь", null=True, blank=True)
	product_pocket = models.ForeignKey('ProductPocket', on_delete=models.CASCADE, verbose_name="Товар" , null=True, blank=True)
	created = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания", null=True, blank=True)

	def __str__(self):
		return f"{self.user} купил {self.product_pocket}"
...
class Payment(models.Model):
    ...
    status = models.CharField(max_length=10, choices=PAYMENT_STATUS_CHOICES, default='pending', verbose_name="Статус", null=True, blank=True)
