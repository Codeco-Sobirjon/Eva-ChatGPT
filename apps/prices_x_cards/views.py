...
from .models import ProductPocket, ToBuyerUser, Card, Payment
from apps.prices_x_cards.serializers import ProductPocketSerializer, ToBuyerUserSerializer, CardSerializer, \
...
class ToBuyerUserListCreateAPIView(APIView):
    permission_classes = [IsAuthenticated]

    @swagger_auto_schema(
	    tags=['ToBuyerUser'],
        operation_description="Получить список всех покупок пользователей",
        responses={200: ToBuyerUserSerializer(many=True)}
    )
    def get(self, request):
        buyer_users = ToBuyerUser.objects.all()
        serializer = ToBuyerUserSerializer(buyer_users, many=True, context={'request': request})
        return Response(serializer.data)

    @swagger_auto_schema(
	    tags=['ToBuyerUser'],
	    operation_description="Создать новую покупку пользователя",
        request_body=ToBuyerUserSerializer,
        responses={201: ToBuyerUserSerializer}
    )
    def post(self, request):
        serializer = ToBuyerUserSerializer(data=request.data, context={'request': request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
